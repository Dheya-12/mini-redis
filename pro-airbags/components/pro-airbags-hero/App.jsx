...
function Copy({bus}) {
  const fx = useFx(), ref = useRef(null);
  useLayoutEffect(() => {
    const el = ref.current, start = el.querySelector('.btn-start'), svc = el.querySelector('.btn-services');
...
function Srs({bus}) {
  ...
  return (
    <div ref={ref} className="srs" style={box(...P)} aria-label="SRS system status" role="group">
  ...
  return (
    <div ref={ref} className="cards" id="services">
      {CARDS.map(([title, sub, key], i) => {
        ...
        return (
          <a key={key} className="card" href={'#' + key} style={{left: bx, top: by, width: bw, height: bh}} aria-label={title + ', ' + sub}>
...
function Trust() {
  ...
  return (
    <div ref={ref} className="trust">
      {TRUST.map(([t, s], i) => (
        <div key={t} className="trust-item" style={box(M.trust[i].icon[0] - 8, 928, M.trust[i].sub[1] + 24, 984)}>
          <Sprite s={SPRITES['trust_' + i]} ox={M.trust[i].icon[0] - 8} oy={928} />
          <T id={'trust_t' + i} ox={M.trust[i].icon[0] - 8} oy={928} className="trust-title">{t}</T>
          <T id={'trust_s' + i} ox={M.trust[i].icon[0] - 8} oy={928} className="trust-sub">{s}</T>
        </div>
      ))}
...
export default function Hero() {
  ...
  useLayoutEffect(() => {
    ...
    const onResize = () => { const w = heroRef.current.clientWidth; const s = w / W; setScale(s); if (f) f.setSize(s); };
    onResize();
    const ro = new ResizeObserver(onResize); ro.observe(heroRef.current);
    const io = new IntersectionObserver(e => f && f.setRunning(e[0].isIntersecting)); io.observe(heroRef.current);
    return () => { ro.disconnect(); io.disconnect(); f && f.destroy(); };
  }, []);
  ...
  useEffect(() => {
    ...
    bus.navigate = item => {
      ...
      fx.pulse(origin, s, {onArrive: () => {
        ...
        gsap.fromTo(item.chars, {y: 0}, {keyframes: [{y: -4, textShadow: '0 0 14px rgba(255,60,40,1)', duration: 0.14}, {y: 0, textShadow: '0 0 0 rgba(0,0,0,0)', duration: 0.5, ease: 'power3.out'}], stagger: {each: 0.03, from: 'center'}});
  ...
  return (
    <FX.Provider value={fxRef}>
      <section ref={heroRef} className="hero booting" id="top" style={{height: H * scale}}>
