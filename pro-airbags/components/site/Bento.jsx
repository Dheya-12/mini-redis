...
const TILE = 'relative rounded-[30px] outline outline-white/60 md:rounded-3xl sm:m-0 sm:aspect-[1.45] sm:h-auto sm:w-full sm:overflow-hidden sm:rounded-[20px] sm:outline-4 xs:aspect-[1.65] overflow-hidden';
...
const IMG = 'pointer-events-none absolute inset-0 h-full w-full object-cover';
const SCRIM = 'pointer-events-none absolute inset-x-0 top-0 h-[62%] bg-[linear-gradient(180deg,#0C0C0D_0%,rgba(12,12,13,0.7)_45%,rgba(12,12,13,0)_100%)]';

function Tile({ id, className, img, pos = 'object-center', title, body, cap = '' }) {
  return (
    <li id={id} className={TILE + ' scroll-mt-24 bg-grey-2 ' + className}>
      <p className={CAP + ' ' + cap}><strong className="font-medium text-white">{title}</strong> {body}</p>
      <img alt="" className={IMG + ' ' + pos} decoding="async" loading="lazy" src={img} />
      <div aria-hidden="true" className={SCRIM} />
    </li>
  );
}

/* huly.io "Huly MetaBrain" bento: light backdrop, outlined rounded tiles, decorative line art. */
    ...
    <div className="bg-[#f6f6f6]">
      <section className="source-of-truth relative overflow-hidden pb-[180px] pt-[247px] xl:py-32 md:pb-24 md:pt-[100px] sm:pb-[72px] sm:pt-16" id="specialties">
        <div className="container-wide relative z-10 xl:max-w-[864px] md:max-w-3xl xs:max-w-md">
          <Reveal>
            <h2 className="ml-64 font-title text-80 font-semibold leading-h2 tracking-tighter text-black xl:ml-64 xl:max-w-[450px] xl:text-80 xl:leading-none md:ml-56 md:max-w-none md:text-54 sm:ml-0 sm:text-36">Every part of the system</h2>
            <p className="ml-64 mt-5 max-w-[640px] text-18 leading-tight tracking-tight text-grey-30 xl:ml-64 xl:mt-3 md:ml-56 md:mt-3 sm:ml-0 sm:mt-2 sm:text-16">
              Four specialties, one bay. Each piece of the restraint system is handled by the technician who does it every day, and the whole system is verified together.
            </p>
          </Reveal>
          <div className="mt-14 flex w-full gap-4.5 xl:mt-12 xl:flex-col xl:gap-3.5 md:mt-10 sm:mt-6 sm:gap-3">
            <ul className="flex w-[808px] flex-wrap items-center gap-4.5 xl:w-full xl:justify-between xl:gap-3.5 md:gap-4 sm:grid sm:grid-cols-2 sm:gap-3 xs:grid-cols-1">
              <Tile id="airbag-replacement" className="ml-64 h-[250px] w-[270px] md:ml-56 md:h-[218px] md:w-[234px]" img="/assets/img/pa-wheel.jpg" pos="object-[50%_40%]" title="Airbag replacement." body="New OEM-spec bags, new hardware, new covers." cap="sm:max-w-[240px]" />
              <Tile id="clock-spring-repair" className="outline-4 xl:h-[250px] xl:w-[250px] md:h-[218px] md:w-[230px] h-[250px] w-[250px]" img="/assets/img/pa-module.jpg" title="Clock spring repair." body="Steering wheel wiring restored." cap="xl:pr-[21px] sm:max-w-[260px]" />
              <Tile id="seat-and-curtain-airbags" className="h-[232px] w-[270px] md:h-[202px] md:w-[234px]" img="/assets/img/pa-cabin.jpg" pos="object-[60%_50%]" title="Seat and curtain airbags." body="Side and roof rail systems rebuilt." cap="md:max-w-[195px] sm:max-w-[280px]" />
              <Tile id="pretensioner-rebuild" className="outline-4 xl:w-[515px] md:w-[453px] h-[232px] w-[520px]" img="/assets/img/pa-seatbelt.jpg" pos="object-[50%_30%]" title="Pretensioner rebuild." body="A new charge in your original belt, so trim and color still match." cap="max-w-[425px] xl:px-5 sm:max-w-[290px]" />
            </ul>
            <ul className="flex flex-1 flex-wrap items-center gap-4.5 xl:flex-nowrap xl:items-end sm:grid sm:grid-cols-2 sm:gap-3 xs:grid-cols-1">
              <Tile id="webbing-replacement" className="h-[236px] w-[236px] xl:h-[250px] xl:w-[270px] md:h-[218px] md:w-[235px]" img="/assets/img/pa-belt-sq.jpg" title="Webbing replacement." body="Only when cut, frayed or blown." />
              <Tile id="buckle-and-sensor-repair" className="outline-4 xl:h-[250px] xl:w-[250px] md:h-[218px] md:w-[218px] h-[250px] w-[250px]" img="/assets/img/pa-front.jpg" title="Buckle and sensor repair." body="Buckle switches and impact sensors." cap="sm:max-w-60" />
              <Tile id="retractor-repair" className="h-[232px] w-[422px] outline-4 xl:h-[250px] xl:w-[250px] md:h-[218px] md:w-[218px]" img="/assets/img/pa-screen.jpg" title="Retractor repair and system setup." body="Locked retractors freed, then a full readiness check." cap="max-w-[85%] xl:max-w-full xl:pr-[5px] md:pr-[9px] sm:max-w-[260px]" />
            </ul>
          </div>
        </div>
...
const SENTENCE = <>
  <span className="font-semibold">Every report from Pro Airbags</span> lists what deployed, what was replaced, what was reset and the readiness result, so the adjuster, the body shop and the owner all read the same page.
</>;
...
/* huly.io "Knowledge at Your Fingertips" section: the document-editor demo with pins, toolbar, highlight and cursor, plus the sticky side card. */
function Knowledge() {
  return (
    <section className="knowledge relative mt-[50px] pb-[247px] px-safe lg:mt-0.5 lg:pb-[102px] md:pb-[94px] sm:pb-[72px]" id="report">
      <div className="container-wide relative grid grid-cols-[264px_1fr] gap-x-[181px] xl:grid-cols-[337px_1fr] xl:gap-x-[108px] lg:grid-cols-[256px_1fr] lg:gap-x-16 md:grid-cols-[177px_1fr] md:gap-x-[37px] sm:grid-cols-1">
          ...
          <Reveal className="doc-demo" threshold={0.15}>
            <h2 className="relative mt-[-40px] max-w-xl pt-[40px] font-title text-80 font-semibold leading-none tracking-tighter text-black lg:text-80 md:mt-[-30px] md:pt-[30px] md:text-54 sm:text-36 2xs:max-w-[268px]">
              ...
              <span className="relative">
                <span className="invisible">documented</span>
                <span aria-hidden="true" className="absolute left-0 top-7 lg:top-5 sm:top-2.5">
                  <span />
                  <Pin color="blue" img="/assets/img/pa-logo.jpg" line="h-[46px] md:h-[34px] sm:h-[26px]" className="right-[-38px] top-[-38px] text-blue lg:-right-10 lg:-top-10 md:-right-7 md:-top-8 sm:right-[-22px]" />
                </span>
              </span>
            </h2>
            <p className="mt-10 lg:mt-9 md:mt-4 sm:mt-3">
              A restraint repair is only finished when it is written down. Pro Airbags hands every owner, adjuster and body shop the same readiness report, and keeps a copy with the VIN.
            </p>
            <div className="relative mt-[26px] lg:mt-[23px] md:mt-[20px] sm:mt-[22px]">
              ...
              <div className="relative">
                <span className="relative z-10 md:pr-2">{SENTENCE}</span>
            ...
            <img alt="" className="mt-9 rounded-[10px] lg:mt-8 md:mt-7 md:h-auto md:w-full sm:mt-[19px]" decoding="async" height="423" loading="lazy" src="/assets/img/pa-car.jpg" style={{ color: 'transparent' }} width="706" />
            ...
            <p className="relative -mt-8 pb-3 pt-[68px] lg:-mt-9 md:mt-[-19px] md:pt-[47px] sm:mt-[-22px] sm:pt-[38px]">
              <span aria-hidden="true" className="absolute left-1/2 top-0 -z-10 h-full w-px" />
              With{' '}
              <span className="doc-underline relative bg-gradient-to-br from-grey-70 to-grey-70 bg-[left_14px] bg-no-repeat [transition:background_1s_0.5s,color_0.5s_0s] lg:bg-[left_12px] md:bg-[left_9px] bg-[length:0%_2px] text-grey-20 sm:bg-[length:0%_1px]">every code</span>
              {' '}on the report, the insurer sees the deployment, the parts and the reset in one place. Warranty terms, the road-test result and the technician&apos;s sign-off are the{' '}
              <span className="doc-underline relative bg-gradient-to-br from-grey-70 to-grey-70 bg-[left_14px] bg-no-repeat [transition:background_1.5s_2s,color_0.5s_1.5s] lg:bg-[left_12px] md:bg-[left_9px] bg-[length:0%_2px] text-grey-20 sm:bg-[length:0%_1px]">last three lines</span>
              {' '}
              <span className="relative pr-2 md:pr-1">
                <strong className="font-medium text-orange" />
                <span className="relative">
                  <Pin color="orange" img="/assets/img/pa-belt-sq.jpg" line="h-[27px] md:h-[18px]" className="bottom-[-8px] left-[-30px] text-orange md:bottom-[-4px] md:left-[-21px] sm:left-[-16px]" />
                </span>
              </span>
              of every document we hand over.
            </p>
            <img alt="" className="mt-[22px] rounded-[10px] lg:mt-[22px] md:mt-[11px] md:h-auto md:w-full sm:mt-[3px]" decoding="async" height="336" loading="lazy" src="/assets/img/pa-dash.jpg" style={{ color: 'transparent' }} width="704" />
            <p className="mt-9 lg:mt-8 md:mt-6 sm:mt-4">
              Reports can be sent as PDF, attached to the claim, or printed for the glovebox. Mail-in module customers get theirs by email the day the module ships back.
            </p>
        ...
        <div className="order-first ml-2 pt-[222px] xl:ml-[81px] lg:ml-[63px] lg:pt-[202px] md:ml-6 md:pt-32 sm:hidden">
          <div className="sticky top-[222px] z-10 h-[282px] w-64 overflow-hidden rounded-xl bg-grey-2 ring-1 ring-black/5 lg:top-[202px] lg:h-[227px] lg:w-[206px] md:top-32 md:h-[169px] md:w-[154px]">
            <img alt="" className="absolute inset-0 h-full w-full object-cover opacity-70" decoding="async" loading="lazy" src="/assets/img/pa-module.jpg" />
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(12,12,13,0)_20%,#0C0C0D_80%)]" />
            <div className="absolute bottom-0 left-0 px-4.5 pb-7 pt-4 transition-opacity duration-300 ease-in-out lg:pb-6 lg:pl-4 lg:pr-3 md:px-2.5 md:py-4 z-20 text-white">
              <h3 className="font-semibold leading-snug tracking-tight lg:text-13 md:text-10">Readiness report</h3>
              <p className="mt-1.5 text-15 leading-snug tracking-tight lg:text-12 md:mt-1 md:text-9">Every code, every part, every reset. Signed by the technician.</p>
            </div>
          </div>
